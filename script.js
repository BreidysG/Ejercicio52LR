...
let interval;
let currentCircle = 1;
const timeLimit = 180; // 2 minutes in seconds
...
function startGame() {
    ...
    interval = setInterval(() => {
        currentCircle++;
        if (currentCircle > 10) currentCircle = 1;
        displayCircles(currentCircle);
    }, speed);
}
...
function displayCircles(circleNumber) {
    ...
    const sizes = [50, 100, 150, 200, 250, 300, 350, 400, 450, 500]; // Tamaño de los círculos
...
function resetGame() {
    ...
    timerElement.innerText = 'Tiempo restante: 03:00';
    clearInterval(interval);
    currentCircle = 1;
}
